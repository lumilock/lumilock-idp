...
export class UsersInfosDTO implements Readonly<UsersInfosDTO> {
  ...
  public static from(
    dto: Partial<UsersInfosDTO>,
    validate = false,
  ): UsersInfosDTO {
    const user = new UsersInfosDTO();
    user.id = dto.id;
    user.login =
      dto.login ??
      noTilde(dto.login)
  ...
  public static fromEntity(entity: User, password = true) {
    return this.from({
      ...
      permissions: entity?.usersClients?.[0]?.permissions,
      role: entity?.usersClients?.[0]?.role,
